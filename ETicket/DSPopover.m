...
+ (void)showLoading:(BOOL)show parentView:(UIView *)parentView animate:(BOOL)animate {
    if (!show) {
        ...
        if (animate) {
            DSLoadingSpinner *spinner = [loadingPopover viewWithTag:kSpinnerViewTag];
            if (spinner) {
    ...
    DSLoadingSpinner *spinner = [[DSLoadingSpinner alloc] initWithStrokeColor:[UIColor whiteColor] strokeWidth:2];
