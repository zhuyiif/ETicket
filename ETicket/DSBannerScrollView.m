...
@implementation DSBannerScrollView {
    UIView *_contentView;
...
- (void)setupViews {
    ...
    _pageIndicator = [[DSPageIndicator alloc] init];
...
- (void)onBannerTapped:(UITapGestureRecognizer *)gesture {
    ...
    DSBannerInfo *item = _model[idx];
    if ([NSString isBlankString:item.link]) {
        return;
    }
    
    UIViewController *viewController = [[DSWebViewController alloc] initWithURL:item.link title:item.name];
