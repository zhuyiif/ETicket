...
@implementation ETAlipayOrder

+ (ETAlipayOrder *)orderWithTitle:(NSString *)title body:(NSString *)body amount:(NSNumber *)amount tradeNo:(NSString *)tradeNo {
    if (!amount || !title || !body) {
        return nil;
    }
    ETAlipayOrder *order = [ETAlipayOrder new];
    // NOTE: app_id设置
    order.app_id = kAlipayPartner;
