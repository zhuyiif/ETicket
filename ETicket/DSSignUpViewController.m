...
@interface DSSignUpViewController () <TTTAttributedLabelDelegate,UITextFieldDelegate>

@property (nonatomic, strong) UIView *contentView;
@property (nonatomic, strong) DSEditView *phoneEditView;
@property (nonatomic, strong) DSEditView *nickNameEditView;
@property (nonatomic, strong) DSEditView *verifyCodeEditView;
@property (nonatomic, strong) DSEditView *passwordEditView;
@property (nonatomic, strong) DSEditView *confirmPasswordEditView;
@property (nonatomic, strong) UIStackView *stackView;
@property (nonatomic) TTTAttributedLabel *gobackLabel;
@property (nonatomic, strong) id<RACSubscriber> subscriber;
@property (nonatomic) DSAccountPresenter *presenter;
...
- (UIButton *)submitButton {
    UIButton *submitButton = [UIButton buttonWithStyle:DSButtonStyleOrange height:44];
...
- (UIStackView *)stackView {
    if (!_stackView) {
        ...
        _nickNameEditView = [DSEditView inputViewWithStyle:DSEditViewStyleNormal placeHolder:NSLocalizedString(@"昵称", nil)];
        
        _verifyCodeEditView = [DSEditView inputViewWithStyle:DSEditViewStyleCaptcha placeHolder:NSLocalizedString(@"短信验证码", nil)];
        @weakify(self);
        ...
        _passwordEditView = [DSEditView inputViewWithStyle:DSEditViewStyleInputPassWord placeHolder:NSLocalizedString(@"密码大于6位字符", nil)];
        _confirmPasswordEditView = [DSEditView inputViewWithStyle:DSEditViewStyleInputPassWord placeHolder:NSLocalizedString(@"密码大于6位字符", nil)];
...
- (void)submit {
    if (!self.checkBox.selected) {
        [DSPopover showFailureWithContent:NSLocalizedString(@"请先同意服务条款", nil)];
        return;
    }
    ...
    [DSPopover showLoading:YES];
    DSSignupItem *item = [DSSignupItem new];
    ...
    @weakify(self);
    [[[[DSActor instance] signupWithItem:item] flattenMap:^RACStream *(id value) {
        @strongify(self);
        NSString *password = [self.passwordEditView text];
        NSString *userName = [self.phoneEditView text];
        return [[DSActor instance] loginWithAccount:userName password:password];
    }] subscribeNext:^(id x) {
        @strongify(self);
        [DSPopover showSuccessWithContent:NSLocalizedString(@"注册成功", nil)];
